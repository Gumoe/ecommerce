...
import { confirmedOrder, deleteOrder, deliveredOrder, getOrder, shipOrder } from '../../State/Admin/Order/Action'
const OrdersTable = () => {
            ...
            <TableBody>
              {adminOrder.orders?.map((item,index) => (
                ...
                >
                  <TableCell align="left">
                    <AvatarGroup max={3} sx={{ justifyContent: "start" }}>
                      {item.orderItems.map((orderItem) => <Avatar src={orderItem.
                        products.imageUrl}></Avatar>)}
                    </AvatarGroup>

                  </TableCell>
                  <TableCell align="left" scope="row">
                    {item.orderItems.map((orderItem) => <p >{orderItem.products.title}</p>)}
                    {/* {item.title} */}
                  </TableCell>
                  ...
                  ${item.orderStatus === "CONFIRMED" ? "bg-[#45a845]" :
                      item.orderStatus === "SHIPPED" ? "bg-[#4444ff]" :
                        item.orderStatus === "PLACED" ? "bg-[#10A881]" :
